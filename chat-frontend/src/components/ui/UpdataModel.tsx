...
const UpdateModel = ({ isOpen,  message, currentMessage, onConfirm, onCancel }: ConfirmModelProps) => {
...
const patchMessage = async () => {
  ...
  return createPortal(
   ...
   {   loading?<div><AiOutlineLoading className="animate-spin text-lg" /></div>:<div className={` bg-white p-4 flex  flex-col gap-2 rounded-lg relative max-w-xs  w-full min-w-0`}>
      <IoClose onClick={onCancel} className="cursor-pointer absolute right-2 top-2 text-lg"/>

        <h2 className={`${groupingError}`}>update message</h2>
