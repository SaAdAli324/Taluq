...
const ConfirmModel = ({ isOpen, title, message, onConfirm, onCancel ,loading }: ConfirmModelProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-9999 bg-black/40 flex items-center justify-center">
      <div className="bg-white p-4 rounded-lg max-w-xs w-full min-w-0">
        <h2 className="text-lg font-medium">{title}</h2>
        <p className="text-sm text-gray-500">{message}</p>
        { loading?<div><AiOutlineLoading className="animate-spin text-lg" /></div> : <div className="flex justify-end gap-2 mt-4">
          <button
            onClick={onCancel}
